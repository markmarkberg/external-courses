...
getMock.forEach((card) => {
  ...
  if (card.title !== "Backlog") {
    ...
    // button.setAttribute("onclick", "addList(event)");
  } else {
...
getMock[1].issues.forEach((el) => {
  const div = document.createElement("div");
  div.classList.add("content_item");
  document.querySelector(".ready_task").append(div);
  div.setAttribute("id", el.id);
  div.innerHTML = el.name;
});
getMock[2].issues.forEach((el) => {
  const div = document.createElement("div");
  div.classList.add("content_item");
  document.querySelector(".inprogress_task").append(div);
  div.setAttribute("id", el.id);
  div.innerHTML = el.name;
});
getMock[3].issues.forEach((el) => {
  const div = document.createElement("div");
  div.classList.add("content_item");
  document.querySelector(".finished_task").append(div);
  div.setAttribute("id", el.id);
  div.innerHTML = el.name;
});
...
input.addEventListener("blur", (e) => {
  ...
  let createNum = String(Date.now());

  let createId = createNum.slice(8);
...
document.addEventListener("click", (event) => {
  if (event.target.parentNode.classList.contains("ready_add")) {
    ...
    getMock[0].issues.forEach((el, i) => {
      const option = document.createElement("option");
      option.setAttribute("class", "opt");
      option.innerHTML = el.name;
      option.setAttribute("value", el.id);
      document.querySelector(".ready_drop select").append(option);
    });
    select.onchange = (e) => {
  ...
  if (event.target.parentNode.classList.contains("inprogress_add")) {
    ...
    getMock[1].issues.forEach((el, i) => {
      ...
      option.innerHTML = el.name;
      option.setAttribute("value", el.id);
      document.querySelector(".inprogress_drop select").append(option);
    });
    select.onchange = (e) => {
  ...
  if (event.target.parentNode.classList.contains("finished_add")) {
    ...
    select.classList.remove("item_progress");
    select.classList.remove("item_ready");
    getMock[2].issues.forEach((el, i) => {
      const option = document.createElement("option");
      option.setAttribute("class", "opt");
      option.innerHTML = el.name;
      option.setAttribute("value", el.id);
      document.querySelector(".finished_drop select").append(option);
    });
    select.onchange = (e) => {
